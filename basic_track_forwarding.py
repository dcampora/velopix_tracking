...
class HitsIterator(object):
  ...
  def __getitem__(self, i):
    ...
    if index < 0 or index >= self.last_index:
      raise IndexError
    return (self.hit_X[index], self.hit_Y[index], self.hit_Z[index], self.hit_ID[index])

  def __next__(self):
    self.index += 1
    if self.index >= self.last_index:
      raise StopIteration
    return (self.hit_X[self.index], self.hit_Y[self.index], self.hit_Z[self.index], self.hit_ID[self.index])
...
for s0, s1, starting_sensor_index in zip(reversed(sensors[3:]), reversed(sensors[1:-2]), reversed(range(0, 49))):
  for h0 in s0 if h0[3] not in used_hits:
    for h1 in s1 if h1[3] not in used_hits:
      
      if are_compatible(h0, h1):
        # We have a seed, let's attempt to form a track
        # with a hit from the following three sensors

        for sensor_index in reversed(range(starting_sensor_index-2, starting_sensor_index+1)) if sensor_index >= 0:
